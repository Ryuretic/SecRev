...
class Ryuretic_coupler(coupler):
    ...
    def handle_icmp(self,pkt):
        ...
        if pkt_status is 'flagged':
            ...
            fields,ops = self.Icmp_Redirect(pkt)
        else:
            print "*******\nrespond to ping"
    ...
    def handle_udp(self,pkt):
        print "*\n*\nhandle udp\nhandl udp\nhandle udp"
        print pkt
        #check whether destination is flagged
        pkt_status = self.check_net_tbl(pkt['dstmac'])
        print "Packet status is: ", pkt_status
        fields, ops = self.default_Field_Ops(pkt)
        if pkt_status is 'flagged':
            print "Forwarding UDP and Poisoing Host"
            self.install_field_ops(pkt, fields, ops)
            fields,ops = self.ARP_after_DNS(pkt)
            self.install_field_ops(pkt,fields,ops)
        else:
            self.install_field_ops(pkt, fields, ops)
